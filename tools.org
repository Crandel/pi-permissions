...
:END:
This document lists all tools available to the coding agent, along with
...
gif, webp, bmp). Images are sent as attachments. For text files, output
is truncated to 2000 lines or 50KB (whichever is hit first). Use
...
:CUSTOM_ID: ast_edit
:END:
Structural code search-and-rewrite using ast-grep. Matches code by AST
pattern (not text), so it survives formatting/whitespace differences and
understands language syntax. Always previews a diff and asks for
confirmation before writing, unless =autoApply= is set to true.
...
** lens_diagnostics
:PROPERTIES:
:CUSTOM_ID: lens_diagnostics
:END:
Query pi-lens's diagnostic state. =mode=delta/all= are cache-only and
instant; =mode=full= is an expensive active project-wide check.
...
** lsp_diagnostics
:PROPERTIES:
:CUSTOM_ID: lsp_diagnostics
:END:
Get errors, warnings, and hints from language servers for a file or
directory. Use BEFORE running builds to proactively check for issues.
Works on directories by auto-detecting file extensions and scanning all
matching files.
...
** symbol_search
:PROPERTIES:
:CUSTOM_ID: symbol_search
:END:
Ranked identifier search over the persisted word index (BM25 + priors
demoting tests/vendor/docs) --- answers “which files are most relevant
to =<query>=” by identifier. First step of the discovery funnel:
=symbol_search= finds candidates, =module_report= explains the file,
=read_symbol= reads the body. Complements grep (raw substrings) and
=lsp_navigation= (exact references).
...
discovery funnel: =project_report= orients, =module_report= explains a
file, =read_symbol= reads a body.
...
doc comment is attached), important inline callbacks/closures/lambdas
with stable handles, plus who-uses-this, risk flags, and ranked
recommendedReads.
...
** read_symbol
:PROPERTIES:
:CUSTOM_ID: read_symbol
:END:
Return the verbatim source of a single named symbol or =module_report=
callback handle in a file --- a targeted, cheap alternative to reading
the whole file. Pair with =module_report=: =module_report= finds the
symbol/callback handle, =read_symbol= shows its body.
...
** read_enclosing
:PROPERTIES:
:CUSTOM_ID: read_enclosing
:END:
Return the verbatim source for the smallest useful symbol/callback
enclosing a line in a file. Use after =ast_grep_search=, diagnostics, or
LSP locations when you need exact body text without reading the whole
file.
...
:PROPERTIES:
:CUSTOM_ID: pi_lens_activate_tools
:END:
Activate one or more situational pi-lens tools that stay registered but
inactive by default, so the default tool list stays lean. Call this ONCE
with the tools you need before using them --- they become callable
...
** web_search
:PROPERTIES:
:CUSTOM_ID: web_search
:END:
Search the web via DuckDuckGo. Returns titles, URLs, and snippets. All
content is sanitized against prompt injection before being returned.
...
:CUSTOM_ID: web_fetch
:END:
Fetch and extract the text content of a URL. Blocks private/internal
network addresses (SSRF protection). All content is sanitized against
prompt injection before being returned.
