...
export default function (pi: ExtensionAPI) {
	...
	pi.on("session_start", async (_event, ctx) => {
		...
		log(
			"INFO",
			`Session started. Loaded ${sortedRules.length} rules. Debug mode: ${debug}`,
		);
	...
	pi.on("tool_call", async (event, ctx) => {
		...
		if (action === "allow") {
			if (debug)
				log("DEBUG", `Rule matched: allow (priority: ${rule.priority ?? 0})`);
			return undefined;
		}
		...
		if (debug) {
			log(
				"INFO",
				`User selected: ${userResult.selection} for tool ${event.toolName}`,
			);
		}
