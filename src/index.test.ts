...
test("direct deny emits permissions:deny with source rule and no input", async () => {
	...
	const result = await app.toolCall(
		{ command: "rm -rf /tmp/actual-secret" },
		"deny-1",
	);
	...
	assert.equal(
		JSON.stringify(app.emitted).includes("/tmp/actual-secret"),
		false,
	);
...
test("user deny emits user_select then permissions:deny with source user", async () => {
	...
	assert.deepEqual(
		app.emitted.map((event) => event.name),
		["permissions:ask", "permissions:user_select", "permissions:deny"],
	);
